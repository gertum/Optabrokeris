...
class FixedSimpleXLSXEx
{
    ...
    public function __construct(FixedSimpleXLSX $xlsx)
    {
        $this->xlsx = $xlsx;
        ...
        foreach ($this->xlsx->sheetRels as $index => $xml) {
            foreach ($xml->Relationship as $rel) {
                ...
                if ($rel_type === 'comments') {
                    ...
                    $com_file = FixedSimpleXLSX::getTarget($d, $rel_target);
                    if ($com_xml = $this->xlsx->getEntryXML($com_file)) {
                        foreach ($com_xml->commentList->comment as $com) {
                            $this->comments[$index][(string)$com['ref']] = FixedSimpleXLSX::parseRichText($com->text);
                        }
                    }
                }
            }
        ...
        foreach ($this->xlsx->sheetRels as $index => $xml) {
            ...
            if (isset($sheet->hyperlinks)) {
                foreach ($sheet->hyperlinks->hyperlink as $hyperlink) {
                    ...
                    if (FixedSimpleXLSX::strpos($ref, ':') > 0) { // A1:A8 -> A1
                        $ref = explode(':', $ref);
                        $ref = $ref[0];
                    }
