...
/**
 *    FixedSimpleXLSX php class
 *    MS Excel 2007+ workbooks reader
 *
 * Copyright (c) 2012 - 2022 FixedSimpleXLSX
 *
 * @category   FixedSimpleXLSX
 * @package    FixedSimpleXLSX
 * @copyright  Copyright (c) 2012 - 2022 FixedSimpleXLSX (https://github.com/shuchkin/simplexlsx/)
 * @license    MIT
 */

/** Examples
 *
 * use Shuchkin\FixedSimpleXLSX;
 *
 * Example 1:
 * if ( $xlsx = FixedSimpleXLSX::parse('book.xlsx') ) {
 *   foreach ($xlsx->rows() as $r) {
 *     print_r( $r );
 *   }
 * } else {
 *   echo FixedSimpleXLSX::parseError();
 * }
 *
 ...
 *
 * Example 3: rowsEx
 * $xlsx = FixedSimpleXLSX::parse('book.xlsx');
 ...
 *
 * Example 8: read data
 * if ( $xlsx = FixedSimpleXLSX::parseData( file_get_contents('http://www.example.com/example.xlsx') ) ) {
 ...
 *
 * Example 9: old style
 * $xlsx = new FixedSimpleXLSX('book.xlsx');
 * if ( $xlsx->success() ) {
 *   print_r( $xlsx->rows() );
 * } else {
    ...
    */
    ...
    public function unzip($filename, $is_data = false)
    {
        ...
        if ($is_data) {
            ...
            $this->package['size'] = FixedSimpleXLSX::strlen($filename);
...
//          $bE = false;
            ...
            if ($aP['GPF'] & 0x0008) {
                $aP1 = unpack('V1CRC/V1CS/V1UCS', FixedSimpleXLSX::substr($vZ, -12));

                $aP['CRC'] = $aP1['CRC'];
                $aP['CS'] = $aP1['CS'];
                $aP['UCS'] = $aP1['UCS'];
                // 2013-08-10
                $vZ = FixedSimpleXLSX::substr($vZ, 0, -12);
                if (FixedSimpleXLSX::substr($vZ, -4) === "\x50\x4b\x07\x08") {
                    $vZ = FixedSimpleXLSX::substr($vZ, 0, -4);
                }
            }

            // Getting stored filename
            $aI['N'] = FixedSimpleXLSX::substr($vZ, 26, $nF);
            $aI['N'] = str_replace('\\', '/', $aI['N']);

            if (FixedSimpleXLSX::substr($aI['N'], -1) === '/') {
            ...
            $vZ = FixedSimpleXLSX::substr($vZ, 26 + $nF + $mF);

            if ($aP['CS'] > 0 && (FixedSimpleXLSX::strlen($vZ) !== (int)$aP['CS'])) { // check only if availabled
                $aI['E'] = 1;
...
//        $m1 = 0; // memory_get_peak_usage( true );
        ...
        if ($relations = $this->getEntryXML('_rels/.rels')) {
            foreach ($relations->Relationship as $rel) {
                ...
                $rel_target = FixedSimpleXLSX::getTarget('', (string)$rel['Target']); // /xl/workbook.xml or xl/workbook.xml
                ...
                if ($rel_type === 'officeDocument'
                    ...
                    if ($workbookRelations = $this->getEntryXML(dirname($rel_target) . '/_rels/workbook.xml.rels')) {
                        ...
                        foreach ($workbookRelations->Relationship as $workbookRelation) {
                            ...
                            $wrel_target = FixedSimpleXLSX::getTarget(dirname($rel_target), (string)$workbookRelation['Target']);
                                ...
                                if ($sharedStrings = $this->getEntryXML($wrel_target)) {
                                    foreach ($sharedStrings->si as $val) {
                                        if (isset($val->t)) {
                                            ...
                                            $this->sharedstrings[] = FixedSimpleXLSX::parseRichText($val);
                                        }
                                    }
                                }
...
//            echo round( ($m2-$m1) / (1024 * 1024), 2).' MB'.PHP_EOL;
    ...
    public function getEntryData($name)
    {
        ...
        $dir = FixedSimpleXLSX::strtoupper(dirname($name));
        $name = FixedSimpleXLSX::strtoupper(basename($name));
        foreach ($this->package['entries'] as &$entry) {
            if (FixedSimpleXLSX::strtoupper($entry['path']) === $dir && FixedSimpleXLSX::strtoupper($entry['name']) === $name) {
                if ($entry['error']) {
                    return false;
                }
                switch ($entry['cm']) {
                ...
                if (!$entry['error'] && $entry['cm'] > -1) {
                    ...
                    } elseif ($entry['ucs'] > 0 && (FixedSimpleXLSX::strlen($entry['data']) !== (int)$entry['ucs'])) {
                        $entry['error'] = 3;
 ...
 // 0.6.6
    ...
    public function dimension($worksheetIndex = 0)
    {
        ...
        if (FixedSimpleXLSX::strpos($ref, ':') !== false) {
            $d = explode(':', $ref);
    ...
    public function getIndex($cell = 'A1')
    {
        ...
        if (preg_match('/([A-Z]+)(\d+)/', $cell, $m)) {
            ...
            $colLen = FixedSimpleXLSX::strlen($col);
            $index = 0;
    ...
    public function value($cell)
    {
        ...
        switch ($dataType) {
            ...
            case 'inlineStr':
                ...
                $value = FixedSimpleXLSX::parseRichText($cell->is);
...
//            require_once __DIR__ . '/FixedSimpleXLSXEx.php';
            $this->rowsExReader = new FixedSimpleXLSXEx($this);
        }
    ...
    public function getCell($worksheetIndex = 0, $cell = 'A1')
    {
        ...
        if (is_array($cell)) {
            $cell = FixedSimpleXLSX . phpSimpleXLSX::num2name($cell[0]) . $cell[1];// [3,21] -> D21
        }
        if (is_string($cell)) {
    ...
    public static function getTarget($base, $target)
    {
        $target = trim($target);
        if (strpos($target, '/') === 0) {
            return FixedSimpleXLSX::substr($target, 1);
        }
    ...
    public static function num2name($num)
    {
        ...
        $num2 = (int)(($num - 1) / 26);
        if ($num2 > 0) {
            return FixedSimpleXLSX . phpSimpleXLSX::num2name($num2) . $letter;
        }
        return $letter;
    }
